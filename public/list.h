 ...
 *
 * rcsid "$Header: /pa-build1/cvs/bora/vmkernel/public/list.h,v 1.1.1.1 2000/05/24 04:14:13 yoel Exp $ SPRITE (Berkeley)"
 */
...
extern void List_Init(List_Links *headerPtr); 
extern void List_Insert(List_Links *itemPtr, List_Links *destPtr); 
extern void List_Remove(List_Links *itemPtr); 
 ...
 *
 * LIST_ITER is the same except it starts from an arbitrary point, indicated
 *           by the initial value of itemPtr.
 * ----------------------------------------------------------------------------
 */
...
#define LIST_ITER(headerPtr, itemPtr) \
        for (; \
             !List_IsAtEnd((headerPtr),itemPtr); \
             itemPtr = List_Next(itemPtr))
