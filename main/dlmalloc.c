...
#ifdef USE_DL_PREFIX
#define public_cALLOc    DLM_calloc
#define public_fREe      DLM_free
#define public_cFREe     DLM_cfree
#define public_mALLOc    DLM_malloc
#define public_mEMALIGn  DLM_memalign
#define public_rEALLOc   DLM_realloc
#define public_vALLOc    DLM_valloc
#define public_pVALLOc   DLM_pvalloc
#define public_mALLINFo  DLM_mallinfo
#define public_mALLOPt   DLM_mallopt
#define public_mTRIm     DLM_malloc_trim
#define public_mSTATs    DLM_malloc_stats
#define public_mUSABLe   DLM_malloc_usable_size
#define public_iCALLOc   DLM_independent_calloc
#define public_iCOMALLOc DLM_independent_comalloc
#else /* USE_DL_PREFIX */
...
#ifndef MORECORE
#define MORECORE(av, size) (av->moreCore(av->heap, size))
#endif
...
#if __STD_C
Void_t*  public_mALLOc(mstate av, size_t);
#else
...
*/
#if __STD_C
void     public_fREe(mstate av, Void_t*);
#else
void     public_fREe();
#endif
...
*/
#if __STD_C
Void_t*  public_cALLOc(mstate av, size_t, size_t);
#else
...
*/
#if __STD_C
Void_t*  public_rEALLOc(mstate av, Void_t*, size_t);
#else
...
*/
#if __STD_C
Void_t*  public_mEMALIGn(mstate av, size_t, size_t);
#else
...
*/
#if __STD_C
Void_t*  public_vALLOc(mstate av, size_t);
#else
...
*/
#if __STD_C
int      public_mALLOPt(mstate av, int, int);
#else
int      public_mALLOPt();
#endif
...
*/
#if __STD_C
struct mallinfo public_mALLINFo(mstate av);
#else
...
*/
#if __STD_C
Void_t** public_iCALLOc(mstate av, size_t, size_t, Void_t**);
#else
...
*/
#if __STD_C
Void_t** public_iCOMALLOc(mstate av, size_t, size_t*, Void_t**);
#else
 ...
 */
#if __STD_C
Void_t*  public_pVALLOc(mstate av, size_t);
#else
...
*/
#if __STD_C
void     public_cFREe(mstate av, Void_t*);
#else
void     public_cFREe();
#endif
...
*/
#if __STD_C
int      public_mTRIm(mstate av, size_t);
#else
int      public_mTRIm();
#endif
...
static Void_t** iALLOc(mstate av, size_t, size_t*, int, Void_t**);
#else
...
#else
static void do_check_remalloced_chunk(av, p, s) mstate av; mchunkptr p; INTERNAL_SIZE_T s;
#endif
{
  INTERNAL_SIZE_T sz = p->size & ~PREV_INUSE;
...
#else
static void do_check_malloced_chunk(av, p, s) mstate av; mchunkptr p; INTERNAL_SIZE_T s;
#endif
{
...
#else
  Void_t* mALLOc(av, bytes) mstate av; size_t bytes;
#endif
{
...
#else
void fREe(av, mem) mstate av; Void_t* mem;
#endif
{
...
#else
Void_t* rEALLOc(av, oldmem, bytes) mstate av; Void_t* oldmem; size_t bytes;
#endif
{
...
#else
Void_t* mEMALIGn(av, alignment, bytes) mstate av; size_t alignment; size_t bytes;
#endif
{
  INTERNAL_SIZE_T nb;             /* padded  request size */
...
#else
Void_t* cALLOc(av, n_elements, elem_size) mstate av; size_t n_elements; size_t elem_size;
#endif
{
  mchunkptr p;
...
#else
void cFREe(av, mem) mstate av; Void_t *mem;
#endif
{
  fREe(av, mem);
}
...
#else
Void_t** iCALLOc(av, n_elements, elem_size, chunks) mstate av; size_t n_elements; size_t elem_size; Void_t* chunks[];
#endif
{
...
#else
Void_t** iCOMALLOc(av, n_elements, sizes, chunks) mstate av; size_t n_elements; size_t sizes[]; Void_t* chunks[];
#endif
{
...
static Void_t** iALLOc(mstate av,
                       ...
                       Void_t* chunks[])
#else
static Void_t** iALLOc(av, n_elements, sizes, opts, chunks) mstate av; size_t n_elements; size_t* sizes; int opts; Void_t* chunks[];
#endif
{
...
#else
Void_t* vALLOc(av, bytes) mstate av; size_t bytes;
#endif
{
  /* Ensure initialization */
  if (av->max_fast == 0) malloc_consolidate(av);
...
#else
Void_t* pVALLOc(av, bytes) mstate av; size_t bytes;
#endif
{
  size_t pagesz;
...
{
   DEBUG_ONLY(struct mallinfo mi = mALLINFo(av);)

  LOG(0, "max system bytes = %10lu\n",
      (CHUNK_SIZE_T)(mi.usmblks));      
  
  LOG(0, "system bytes     = %10lu\n",
      (CHUNK_SIZE_T)(mi.arena + mi.hblkhd));
  LOG(0, "in use bytes     = %10lu\n",
 ...
 *
 * Results:
 *      amount of memory needed for storing dlmalloc state
 *
 * Side effects:
 *      None.
 *
 ...
 */
uint32
DLM_InitHeap(mstate av, Heap_ID heap, Heap_MoreCore moreCore)
{
   memset(av, 0, sizeof *av);
   av->moreCore = moreCore;
   av->heap = heap;
   return sizeof *av;
}
 ...
 * DLM_GetStateSize --
 *
 *      Workaround to get size of malloc_state struct. 
 *
 * Results:
 *      amount of memory needed for storing dlmalloc state
 *
 ...
 *----------------------------------------------------------------------
 *
 * DLM_GetFencepostSize --
 *
 *      The standard HeapCreate and MoreCore functions in heap.c assert that ranges
 *      passed into dlmalloc are MALLOC_ALIGNMENT aligned -- both starting address and
 *      overall length. As such, on each new range added to dlmalloc in sYSMALLOc by
 *      calling MoreCore, this fencepost size amount of memory should be subtracted from
 *      the heap's perception of its current size, since the fencepost memory will never
 *      be free'd / reallocated.
 *
 * Results:
 *      amount of memory needed for putting a fencepost at the end of a range 
 *
 * Side effects:
 *      None.
 *
 ...
 */
uint32
DLM_GetFencepostSize(void)
{
   return ALIGN_UP(3 * SIZE_SZ, MALLOC_ALIGNMENT);
}
 ...
 * DLM_ForEachChunk
 *
 *      Call the given function for each chunk.  If inUseOnly is TRUE, only
 *      call the function for chunks that are in use.
 *
 *	Note that this function is heavily dependent on the fact that the heap.c MoreCore
 *	function allocates a MALLOC_ALIGNMENT sized buffer at the start of each range
 *	which, unbeknownst to dlmalloc, prevents it from ever combining one MoreCore range
 *	with another. This guarantees that the memory pointer passed in is a valid chunk.
 *
 * Results:
 *      None
 *
		...
		uint32 len)
{
   ...
   assert(aligned_OK(chunk2mem(mem)));
   
   /* get rid of fastbins, so that in-use chunks will really be in-use */
   malloc_consolidate(av);

   /* start of range should be chunk */
   chunkPtr = (mchunkptr)mem;

   /* 
    * Assuming everything goes according to plan, the following while loop condition
    * should never be the reason that the loop is broken... The check for fenceposts for
    * the old range and av->top for the current range should cover all exit cases.
    */
   while ((VA)chunkPtr < (VA)end) {

      /* fencepost detect -- for the end of old ranges, not the current one*/
      if (chunksize(chunkPtr) == SIZE_SZ &&
          chunksize(next_chunk(chunkPtr)) == SIZE_SZ) {
	  break;
      }
     
      check_chunk(av, chunkPtr);

      /* 
       * This detects the end of the most recently added range. This check is especially
       * needed because the av->top portion of memory is marked as in-use, but it's
       * really free space.
       */
      if (chunkPtr == av->top) {
         if (!inUseOnly) {
	    (*callback)(av->heap, FALSE, chunk2mem(chunkPtr), chunksize(chunkPtr));
	 }
	 break;
      }

      if (inuse(chunkPtr)) {
	 (*callback)(av->heap, TRUE, chunk2mem(chunkPtr), chunksize(chunkPtr));
      } else if (!inUseOnly) {
	 (*callback)(av->heap, FALSE, chunk2mem(chunkPtr), chunksize(chunkPtr));
      }
      
      chunkPtr = next_chunk(chunkPtr);
   }
}
 ...
 * DLM_FastAvail
 *
 *      Return an approximatation of available memory that can be computed
 *      very quickly.
 *
 * Results:
 ...
 */
uint32
DLM_FastAvail(mstate av)
{
   return chunksize(av->top);
}
